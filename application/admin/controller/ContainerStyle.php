...
use think\Request;
use app\admin\model\ContainerStyle as ContainerStyleModel;

class ContainerStyle extends Base
{
    public function __construct(Request $request = null)
    {
        parent::__construct($request);
        $this->currentModel = new ContainerStyleModel();
    }
    ...
    public function getDataList()
    {
        ...
        $list = $this->currentModel
            ->alias('s')
            ->where($map)
            ->join('product p', 's.id = p.style_id', 'LEFT')
            ->group('s.id')
            ->field('s.id, s.name, s.create_time, count(p.style_id) as goods_num')
            ->select();
